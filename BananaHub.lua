--[=[
    BananaHub - Secure UI Key System (Blocked Legacy Key)
]=]
...
local BadKey = "A75d3a5efb98f8be19b36d2c"

-- 如果外部嘗試塞入那串會繞過的爛鑰匙，直接強制清空，不給通關
if getgenv().Key == BadKey or (getgenv().Key and getgenv().Key ~= CorrectKey) then
    getgenv().Key = nil
end

-- 如果已經有正確的 Key 才能直接過，否則強制跳出 UI
local verified = (getgenv().Key == CorrectKey)
